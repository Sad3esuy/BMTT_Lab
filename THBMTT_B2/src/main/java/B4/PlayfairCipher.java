...
public class PlayfairCipher {
    ...
    public String decrypt(String ciphertext) {
        ...
        for (int i=0; i < ciphertext.length(); i+= 2) {
            ...
            int[] posl = findPosition(firstChar);
            int[] pos2 = findPosition(secondChar);
            if (posl[0] == pos2[0]) {
                plaintext.append(keyMatrix [posl[0]] [(posl[1] + 4) % 5]); 
                plaintext.append(keyMatrix [pos2[0]] [(pos2[1] + 4) % 5]); 
            } 
            else if (posl[1] == pos2[1]) {
                plaintext.append(keyMatrix[(posl[0] + 4) %5] [posl[1]]);
                plaintext.append(keyMatrix [(pos2 [0] + 4) %5] [pos2[1]]);
            } 
            else {
                plaintext.append(keyMatrix [posl[0]] [pos2[1]]); 
                plaintext.append(keyMatrix [pos2[0]] [posl[1]]);
            }
        }
